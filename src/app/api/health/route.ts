...
export async function GET() {
  ...
  try {
    ...
    try {
      ...
      // Use a basic query that works with any Supabase instance
      const { data, error } = await supabase.from('__test__').select('*').limit(1)
      const dbTime = Date.now() - dbStart
      
      // For now, just check if the connection works (ignore "relation does not exist" error)
      const isConnectionWorking = !error || error.message.includes('relation') || error.message.includes('does not exist')
      
      health.services.database = {
        status: isConnectionWorking ? 'healthy' : 'unhealthy',
        responseTime: dbTime,
        error: isConnectionWorking ? null : error?.message,
        note: error?.message.includes('does not exist') ? 'Database connected, tables not initialized' : null
      }
    } catch (dbError) {
      health.services.database = {
        status: 'unhealthy',
