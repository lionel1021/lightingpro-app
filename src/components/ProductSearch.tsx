  ...
  const renderProductCard = (product: LightingProduct) => (
    ...
    `}>
      <div className={`relative ${viewMode === 'list' ? 'w-48 flex-shrink-0' : ''}`}>
        ...
        {/* Upload Images Button for Admin */}
        {process.env.NODE_ENV === 'development' && (
          <div className="absolute bottom-2 left-2">
            <button
              onClick={(e) => {
                e.stopPropagation();
                // Open image upload modal for this product
                console.log('Upload images for:', product.id);
              }}
              className="p-1 bg-white/90 backdrop-blur-sm rounded-full text-gray-600 hover:text-blue-600 hover:bg-white transition-colors"
              title="上传图片"
            >
